...
function currentWeather() {
    ...
    var apiKey = "http://api.openweathermap.org/data/2.5/weather?q="+ input+ " &units=imperial&APPID="+myKey
...
function toStandardTime(militaryTime) {
    ...
    return (militaryTime[0].charAt(0) == 1 && militaryTime[0]) ? (militaryTime[0] - 12) + ':' + militaryTime[1] + ' P.M.' : militaryTime.join(':') + ' A.M.'
}
...
function futureWeather() {
    ...
    let apiKey = "http://api.openweathermap.org/data/2.5/forecast?q="+ input +"&units=imperial&APPID="+myKey
    ...
    $.ajax({
        ...
        let ribbonDiv = document.getElementById(ribbon)
